...
<div>
	...
	<div class="list-preview" :style='{"width":"100%","padding":"10px 7%","margin":"0px auto","position":"relative","background":"none"}'>
		<div class="category-1" :style='{"border":"1px solid #bdefb6","padding":"20px 20px 20px","boxShadow":"inset 0px 0px 56px 0px #c5f1c0","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","height":"auto"}'>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"30px 10px 0","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto"}'>
      <el-form-item :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}'>
	    <div class="lable" v-if="false" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px"}'>景点名称</div>
        <el-input v-model="formSearch.jingdianmingcheng" placeholder="景点名称" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}'>
		<div class="lable" v-if="false" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px"}'>景点类型</div>
        <el-select v-model="formSearch.jingdianleixing" placeholder="请选择景点类型" :clearable="true">
          <el-option v-for="(item, index) in jingdianleixingOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
	  ...
	  <el-button v-if="isAuth('jingdianxinxi','新增')" :style='{"cursor":"pointer","padding":"0px 10px","margin":"0 4px 0 0","borderColor":"#fda100","color":"#fda100","outline":"none","borderRadius":"4px","background":"#fcf2e1","borderWidth":"1px 4px","width":"auto","fontSize":"14px","lineHeight":"42px","borderStyle":"solid","height":"42px"}' type="primary" @click="add('/index/jingdianxinxiAdd')"><i v-if="true" :style='{"color":"#fda100","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"margin":"20px 0 10px","background":"none"}'>
		...
		<div class="list1 index-pv1" :style='{"padding":"0 10px","margin":"30px 0 0","flexWrap":"wrap","background":"none","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
			<div :style='{"padding":"10px","boxShadow":"0px 2px 6px #e6e6e6","margin":"0 0 20px","borderRadius":"8px","flexWrap":"wrap","background":"#fff","display":"flex","width":"23.5%","position":"relative","justifyContent":"space-between","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click="toDetail(item)" class="list-item animation-box">
				<img :style='{"cursor":"pointer","width":"100%","margin":"0 0 8px","objectFit":"cover","display":"block","height":"275px"}' v-if="item.tupian && item.tupian.substr(0,4)=='http'" :src="item.tupian" class="image" />
				<img :style='{"cursor":"pointer","width":"100%","margin":"0 0 8px","objectFit":"cover","display":"block","height":"275px"}' v-else :src="baseUrl + (item.tupian?item.tupian.split(',')[0]:'')" class="image" />
				...
				<div :style='{"width":"100%","padding":"4px 10px","lineHeight":"24px","fontSize":"16px","color":"#249940","textAlign":"center"}' class="name ">{{item.jingdianmingcheng}}</div>
				<div :style='{"width":"100%","padding":"4px 10px","lineHeight":"24px","fontSize":"16px","color":"#249940","textAlign":"center"}' class="name ">{{item.jingdiandengji}}</div>
				<div :style='{"width":"100%","padding":"4px 10px","lineHeight":"24px","fontSize":"16px","color":"#249940","textAlign":"center"}' class="name ">票价:{{item.piaojia}}</div>
				<div :style='{"width":"100%","padding":"4px 10px","lineHeight":"24px","fontSize":"16px","color":"#249940","textAlign":"center"}' class="name ">票数:{{item.piaoshu}}</div>
			</div>
		</div>
...
<script>
  export default {
    ...
    created() {
      ...
      this.$http.get('option/jingdianleixing/jingdianleixing').then(res => {
        if (res.data.code == 0) {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.formSearch.jingdianleixing != '') searchWhere.jingdianleixing = this.formSearch.jingdianleixing;
        if (this.curFenlei != '全部') searchWhere.jingdianleixing = this.curFenlei;
        this.$http.get('jingdianxinxi/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/jingdianxinxiDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
